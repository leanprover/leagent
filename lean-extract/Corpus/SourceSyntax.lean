...
def declarationKey? (fileMap : FileMap) (cmdStx : Syntax) : Option (Nat × Nat) := do
...
def buildSourceMap (source : String) (commands : Array Syntax) :
  ...
  for cmdStx in commands do
    if cmdStx.getKind == ``Lean.Parser.Command.declaration then
      if let some declId := declarationId? cmdStx then
        if let some rawPos := declId[0].getPos? then
          let pos := fileMap.toPosition rawPos
          result := result.insert (pos.line, pos.column) (signatureBodyOf cmdStx source)
  return result
